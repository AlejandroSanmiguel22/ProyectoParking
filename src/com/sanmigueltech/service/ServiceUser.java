package com.sanmigueltech.service;
...
public class ServiceUser {
    ...
    private boolean checkDuplicateCode(String code) throws SQLException {
        ...
        PreparedStatement p = con.prepareStatement("select UserID from user where VerifyCode=? limit 1", ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
    ...
    public boolean checkDuplicateUser(String user) throws SQLException {
        boolean duplicate = false;
        PreparedStatement p = con.prepareStatement("select UserID from user where UserName=? and Status=VerifyCode limit 1");
        p.setString(1, user);
        ResultSet r = p.executeQuery();
        if (r.next()) {
            duplicate = true;
